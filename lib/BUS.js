...
//          })
  ...
  switchRemoteHealthReports (evt) {
    evt = evt || 'on'
    this.healthmonitor[evt]('health.info', this.forward('health.info'))
    this.healthmonitor[evt]('health.status', this.forward('health.status'))
    this.healthmonitor[evt]('health.heartbeat', this.forward('health.heartbeat'))
    this.healthmonitor[evt]('health.error.rejection', this.forward('health.error.rejection'))
    this.healthmonitor[evt]('health.error.uncaught', this.forward('health.error.uncaught'))
  }
  ...
  attach (topic) {
    let me = this
    if (this.triggers.indexOf(topic) < 0) {
      this.triggers.push(topic)
    }
    return function () {
      let args = Array.prototype.slice.call(arguments)
      args.unshift(topic)
