...
var Class = require('./BaseClass').extend({
	...
	constructor: 	function( config ) {
						...
						Object.defineProperties(this,{
							...
							/**
							 * @property {Array}
							 * An array of enumerable properties that are explicitly **not** JSON friendly.
							 * @private
							 */
				            _notJson: {
				            	value:		['modified'],
	...
	/**
	 * @method
	 * Creates a JSON data object with no functions. Only uses enumerable attributes of the object.
	 * @param {Object} [obj]
	...
	toJson: function(obj){
		...
		for (var key in _obj) {
			_obj._notJson = _obj._notJson || '';
			if (_obj.hasOwnProperty(key) && _obj._notJson.indexOf(key) < 0) {
				
				var dsc = Object.getOwnPropertyDescriptor(_obj,key);
				
				if (dsc.get)
					try { rtn[key] = typeof _obj[key] == 'object' ? _obj[key].toJson() : _obj[key];} catch(e) {}
				else if (!dsc.set) {
					switch (typeof _obj[key]) {
						case 'function':
							break;
						case 'object':
							struct = _obj[key].hasOwnProperty('toJson') == true ? _obj[key].toJson() : _obj[key];
