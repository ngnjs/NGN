...
var Class = Base.extend({
	...
	define: function(cls, config) {
		...
		if (target == 'NGN.core' || target == '__NGN.core')
			var _base = require('./NGN.core');
		else {
			var _cls 	= target.split('.');

			global[_cls[0]] = global[cls[0]] || {};

			var _base 	= global[_cls[0]];
		...
		return _base.extend({
			...
			constructor: 	config.constructor == undefined 
							? function(cfg){
								cfg = cfg || config || {};
								if (config.initConfig){
									cfg = config.initConfig(cfg);
									delete cfg.initConfig;					
								}
							...
							: function(cfg) {
								...
								var _cons = config.constructor;
								if (cfg.constructor !== undefined)
