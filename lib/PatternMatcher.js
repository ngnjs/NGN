 ...
 * 
 * For example:
 * 		var pattern = new NGN.PatternMatcher();
 * 		
...
var Class = Base.extend({
	...
	constructor: function(config){
		...
		var props = {
			...
			/**
			 * @property {RegExp} [passwordDotNET=/(?=^[!@#$%\^&*()_\-+=\[{\]};:<>|\./?a-zA-Z\d]{7,}$)(?=([!@#$%\^&*()_\-+=\[{\]};:<>|\./?a-zA-Z\d]*\W+){1,})[!@#$%\^&*()_\-+=\[{\]};:<>|\./?a-zA-Z\d]*$/]
			 * Validate strong password for .NET MembershipProvider. Checks for minimum required password length and for minimum required non-alphanumeric characters. Change value '7' according to MinRequiredPasswordLength and '1' to MinRequiredNonAlphanumericCharacters.
			 * [Source](http://www.regexlib.com/)
			 * @private
			 */
			passwordDotNet: {
				value:		config.passwordDotNET || /(?=^[!@#$%\^&*()_\-+=\[{\]};:<>|\./?a-zA-Z\d]{7,}$)(?=([!@#$%\^&*()_\-+=\[{\]};:<>|\./?a-zA-Z\d]*\W+){1,})[!@#$%\^&*()_\-+=\[{\]};:<>|\./?a-zA-Z\d]*$/,
				enumerable:	true,
				writable:	false
			},
			/**
			 * @method isPasswordDotNet
			 * @param {String} text
			 * The string to match.
			 * @returns {Boolean}
			 */
