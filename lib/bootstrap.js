...
require('wrench').readdirSyncRecursive(__dirname).forEach(function( file ){
	if (p.join(__dirname,file) !== __filename
    	...
    	if (file.split(p.sep)[0] === '_fn'){
    		var _fn = p.join(__dirname,file),
    		    fl = file.split(p.sep);
    		fl = fl[fl.length-1];
    		
    		Object.defineProperty(namespace,fl.replace('.js','').toLowerCase(),{
    		  enumerable: true,
    		  get: function(){
    		    return require(_fn);
    		  }
...
Object.defineProperty(global,'__NGN',{
  enumerable: false,
  writable: true,
  configurable: false,
...
Object.defineProperty(global,'__NGNX',{
  enumerable: false,
  writable: true,
  value: __NGN._ngnx || {}
});
...
// Create Models
Object.defineProperty(global.__NGN,'OBJECT_PROXY_SUPPORT',{
  enumerable: false,
  writable: false,
  configurable: false,
...
var str = '|      Running '+'NGN v'.bold+__NGN.version.bold+' on '+('node '+process.version+'      |').bold;
var str2 = '';
