...
var Class = Base.extend({
	...
	_generateRoutes: function(callback){
								    			...
								    			me._server[key](route, function(__req,__res, __next){
							    					...
							    					var fn = function(){
							    						...
							    						this.form	= __req.method.trim().toUpperCase() == 'PUT' 
																	...
																	|| __req.method.trim().toUpperCase() == 'DELETE'
																	? (typeof __req.body === 'string' ? JSON.parse(__req.body) : __req.body)
																	: {};
														...
														for(var el in this.route){
															if (this.form.hasOwnProperty(el))
																delete this.route[el];
															if (this.url.hasOwnProperty(el))
																delete this.route[el];
		...
		for (var i=0; i<this.routes.length; i++) {	
			...
			if (!fs.existsSync(rt) && fs.existsSync(__NGN.path.join(__NGN.rootDir,rt)))
				rt = __NGN.path.join(__NGN.rootDir,rt);
			rt = __NGN.path.resolve(__NGN.rootDir,rt);		
