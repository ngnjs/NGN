 ...
 * for different server types like NGN.http.Server, NGN.http.APIServer, etc.
 * @private
...
var Class = Base.extend({
	...
	constructor: function(config){
      ...
      /**
       * @cfg {Boolean} [autoRefreshRoutes=true]
       * Setting this to `true` turns on a file watcher on the #routes directory.
      ...
      autoRefreshRoutes: {
        ...
        writable: false,
        configurable: false,
        value: __NGN.coalesce(config.autoRefreshRoutes,true)
      },
			...
			/**
			 * @property {Object} _serverOptions
			...
			/**
			 * @property {Object} _router
			 * Holds the router information relevant to the source file.
			...
			/**
			 * @cfg {String[]} [allowCustomHeaders=[]]
			 * One or more custom HTTP headers
			 * explicitly allowed by `Access-Control-Allow-Headers`.
			 * This is most commonly used to support specific headers
			 * from cross-origin sources (ex: `X-Requested-With` or `X-API-Key`).
			 */
			allowCustomHeaders: {
				value:		config.allowCustomHeaders || [],
				enumerable:	true,
				writable:	true
			},
			...
			/**
			 * @cfg {Function/Array/NGN.http.Processor} [processors=[]]
		...
		for(var i=0;i<this.processor.length;i++){
			if (this.processor[i].type == undefined)
				this.processor[i] = new __NGN.http.Processor({fn:this.processor[i]});
		}
		...
		if (!this.allowCustomHeaders instanceof Array)
			this.allowCustomHeaders = this.allowCustomHeaders.split(',');

		if (this.allowCustomHeaders.length > 0)
			this.allowCustomHeader(this.allowCustomHeaders);

    // Setup file watcher
    if (this.autoRefreshRoutes){
      var me = this;
      Object.defineProperty(this,'clearRouteSource',{
        enumerable: false,
        writable: true,
        configurable: false,
        value: function(filename,remove){
          try {
            remove = __NGN.coalesce(remove,false);
            var routes = me._server.routes,
                unaffected = {};
            for (var method in routes){
              if (['get','post','put','delete','options','head','trace'].indexOf(method.toLowerCase()) >= 0){

                var arr = routes[method].slice(0);
                unaffected[method] = unaffected[method] || [];
                unaffected[method] = unaffected[method].concat(arr.filter(function(route){
                  return !route.source;
                }));

                var newRt = routes[method].slice(0).filter(function(route){return route.source && route.source !== filename});
                delete me._server.routes[method];

              }
            }

            UTIL.unrequire(filename);

            if (!remove) {
              var rt = require(filename)
              me._server.map(rt);

              for (var method in me._server.routes){
                for (var route in me._server.routes[method]){
                  Object.keys(rt).forEach(function(path){
                    if (me._server.routes[method][route].path){
                      if (me._server.routes[method][route].path.toString().indexOf(path) == 0){
                        me._server.routes[method][route].source = filename;
                      }
                    }
                  });
                }
            ...
            for (var method in unaffected){
              me._server.routes[method] = (me._server.routes[method]||[]).concat(unaffected[method]||[]);
            }
          } catch (e) {
            me.fireWarning(filename+' failed to hot-load. Error: '+e.message);
          }
        }
      });
  		UTIL.watch.createMonitor(path.resolve(this.routes),function(monitor){
	...
	addprocessor: function(rp){
		if (rp.type == undefined)
			this.processor.push(new __NGN.http.Processor({fn:rp}));
		else
	...
	_generateRoutes: function(callback){
		...
		if (Array.isArray(this.routes)) {
			for (var i=0; i<this.routes.length; i++) {
				...
				if (fs.existsSync(path.resolve(rt)))
					rt = path.resolve(rt);
				else
					rt = path.resolve(path.join(__NGN.cwd,rt));
				...
				for (var n=0;n<filepath.length;n++){
					if (path.extname(filepath[n]).toLowerCase() == '.js' && filepath[n].toString().trim().toLowerCase() !== 'root.js'){
						var _rt = require(path.join(rt,filepath[n]));
            ...
            if (this.autoRefreshRoutes) {
              for (var method in this._server.routes){
                for (var route in this._server.routes[method]){
                  var paths = Object.keys(_rt);
                  for (var i=0; i<paths.length; i++){
                    if (this._server.routes[method][route].path){
                      if (this._server.routes[method][route].path.indexOf(paths[i]) == 0){
                        this._server.routes[method][route].source = path.join(rt,filepath[n]);
                      }
                    }
                  }
                }
				...
				if (!_hasRootRoute && require('fs').existsSync(path.join(rt,'root.js'))) {
					var _rt = require(path.join(rt,'root.js'));
          ...
          if(this.autoRefreshRoutes){
              ...
              for (var route in this._server.routes[method]){
                ...
                for (var i=0; i<paths.length; i++){
                  if (this._server.routes[method][route].path){
                    if (this._server.routes[method][route].path.indexOf(paths[i]) == 0 && paths[i] !== '/'){
                      this._server.routes[method][route].source = path.join(rt,'root.js');
                    }
                  }
                }
              }
	...
	allowCustomHeader: function(header){
		...
		header = header instanceof Array ? header.join() : header.trim();

		http.prototype.writeHead = function () {
		  var default_headers = this.getHeader("Access-Control-Allow-Headers");
		  if (default_headers)
		  	this.setHeader("Access-Control-Allow-Headers", default_headers+(header.length>0?', '+ header:''));
