...
var Class = Base.extend({
        ...
        broadcastEvent:{
          ...
          value: function(eventName,meta){
            if (me.connected){
              ...
              _data.action = eventName;
              me.mechanic.send(_data);
            } else {
      ...
      initialize: {
        ...
        value: function(callback){
          ...
          Object.defineProperties(this,{
             ...
             */
            name: {
              value:    this.name || 'NGN_Application',
              enumerable: true,
              writable: true
            },
		...
		/*var xclient = new NGN.socket.Client({
		  port: 55556,
		  autoConnect: false
		});
		
    xclient.on('connect',function(){
      console.log('Started.');
    });
	...
	_socketHandler: function(socket){
	  ...
	  // Support custom socket
    socket = socket || process.mechanic.client;
    ...
    mechProcs.forEach(function(proc){
      if (proc !== null){
        socket.on(proc,function(){
          console.log('RE-EMITTING --> '.bold.magenta, proc, arguments);
          me.emit(proc,me['on'+__NGN.string.capitalize(proc)]||function(){
            me.fireWarning('on'+__NGN.string.capitalize(proc)+' is not defined.');
          });
        });
      }
    });
  ...
  /**
   * @method
   * Sends registration data to [NGN Mechanic](#!/guide/mechanic).
   * @private
   */	
	sendInfo: function(){
    if (!managed){
  ...
  healthcheck: function(){
    ...
    var msg = {
      ...
      uptime: process.uptime(),
      memory: process.memoryUsage(),
      i: manager._socket.bytesRead,
      o: manager._socket._bytesDispatched
    };
    if (remote){
      var server = getHostUtilization();
      for (var i in server){
        msg[i] = server[i];
      }
    }
     ...
     */
    this.fireEvent('heartbeat',msg);
    this.mechanic.sendMessage(msg);
    this.timer = setTimeout(this.healthcheck,process.mechanic.healthcheckFrequency*1000);
  },
  ...
  bubbleEvent: function( eventName, metadata ) {
    ...
    process.mechanic.fireEvent(eventName, metadata || null);
  },
