var express	= UTIL.require('express',true),
  	tpl = UTIL.require('consolidate',true),
  	Base = require('../core/HttpTransport'),
  	fs = require('fs'),
  	passport = null,
  	self = Class;

/**
 * @class NGN.http.WebServer
...
var Class = Base.extend({
	...
	constructor: function(config){
      ...
      /**
       * @cfg {String} [assetRoute=null]
       * The URL route prefix for static assets.
       * 
       * For example, if this were `/static`, then
       * /`http://domain.com/static/image.png` would render `image.png`
       * from the #assets directory.  
       */
      assetRoute: {
        value:    config.assetRoute || null,
        enumerable: true,
        writable: true
      },
	...
	handleError: function(err, req, res, next){
		...
		if (err) {
		  if (err.code == 'MODULE_NOT_FOUND'){
		    UTIL.require(err.message.replace('Cannot find module \'','').replace(/\'/gi,'').trim(),true);
		  } else {
		    throw err;
		  }
		}
		...
		try {
			...
			if (req.accepts('html')) {
        res.render(s.toString(),{ url: req.url, error: err });
		  	return;
			}
	...
	start: function(){
		...
		if (!this.running) {
			...
			try {
				...
				if (this.assets !== null) {
					if (!(fs.existsSync(this.assets) && fs.existsSync(require('path').join(__NGN.rootDir,this.assets))))
						this.assets = __NGN.path.join(__NGN.rootDir,this.assets);
				  
				  if (this.assetRoute){
				    console.log(this.assetRoute,require('path').resolve(this.assets));
  					this._server.use(this.assetRoute,express.static(require('path').resolve(this.assets)));
  			  } else {
  			    this._server.use(express.static(require('path').resolve(this.assets)));
  			  }
				}
	...
	enableTemplateRendering: function(){
		...
		if (this.views !== null || this.customErrors !== null) {
			...
			if (this.views !== null){
        this._server.set('views',require('path').resolve(this.views));
      }
