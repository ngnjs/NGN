...
/**
 * @class NGN.http.proxy.VirtualHost
...
var Class = Base.extend({
	...
	removeAlias: function(name){
		...
		while (name.length > 0){
			var a = name.shift();
			this.alias.splice(this.alias.indexOf(a),1);
		}
	...
	removeTarget: function(dest){
		...
		while (dest.length > 0){
			var lb = dest.shift();
			this.target.splice(this.target.indexOf(lb),1);
		}
	 ...
	 * The substitution string, as seen in NGN.http.proxy.RewriteRule#substitute
	...
	addRewriteRule: function(method,pattern,substitute,last,position){
		...
		if (!ruleAlreadyExists) {
			...
			if (position)
				this.rules[method].splice(position,0,new __NGN.http.proxy.RewriteRule(opt));
			else
				this.rules[method].push(new __NGN.http.proxy.RewriteRule(opt));
	 ...
	 * last attributes of NGN.http.proxy.RewriteRule to determine which rule to remove. 
	 * @param {String} method
	 * GET, PUT, POST, DELETE, HEAD, or TRACE
	 * @param {RegExp} pattern
	 * The pattern that triggers the rule to reqrite the URL.
    ...
    removeRewriteRuleAt: function(method,index) {
    	index = index || 0;
    	this.rules[method].splice(index,1);
    },
    
    /**
     * @method
     * Determines if the virtual host has the specified rewrite rule 
     */
    hasRewriteRule: function(method,pattern,substitute){
    	for (var rule=0; rule<this.rules[method.toUpperCase()].length; rule++) {
			if (pattern == this.rules[method.toUpperCase()][rule].pattern && substitute == this.rules[method.toUpperCase()][rule].pattern)
				return true;
		}
		return false;
    },
    ...
    /**
     * @method
     * Get the configuration of the virtual host.
     * @returns {Object}
     */
    getConfiguration: function(){
		...
		for (var method in this.rules){
			if (this.rules.hasOwnProperty(method)){
			
				obj.rules[method] = obj.rules[method] || [];

				for (var n=0;n<this.rules[method].length;n++) 
					obj.rules[method].push(this.rules[method][n].getConfiguration());
			}
		}
