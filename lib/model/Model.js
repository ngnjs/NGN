...
var Class = Base.extend({
		...
		for (var field in this.fields){
			if (['id'].indexOf(field) < 0) {
				...
				if (this.fields[field] instanceof __NGN.model.data.Association) {
					...
					this._xref[field] = this.fields[field];
					
					Object.defineProperty(this,field,{
						enumerable:	true,
						get:		function(){
										return this._xref[_field].value;
									},
						set:		function(value){
										this._xref[_field].value = value;
									}
					});
				} else {
					...
					if (dsc.get && !this.fields[key] instanceof __NGN.model.data.Association){
						...
						if (dsc.get) {
							if (_obj[key] !== undefined){
								console.log(key.cyan,'needs to be saved before further saving',_obj[key]);
							}
						} else {
	...
	getChanges: function(serializeAssociations){
		...
		if (this.isNew){
			return  Object.keys(this._xref).length == 0 
					? {create: this.serialize(serializeAssociations)}
					: {create: this.serialize(serializeAssociations), associated: this._xref}
