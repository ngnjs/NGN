...
// The proxy handler prototype responsible for managing data. 
var proxyHandler = {
		get: function(rec, name) {
			return this.target[name];
		},
		has: function(name) {
			return this.target[name] !== undefined;
		},
		set: function(rec, name, value) {
			var delta = {
				type:	this.___originalValues[name] == undefined ? 'create' : 'update',
				property: name,
				value:	value
			};
			
			if (delta.type === 'update')
				delta.oldValue = this.target[name];
			
			this.target.onBeforeChange(delta);
			
			if (!this.___originalValues[name] && delta.type === 'create')
				this.___originalValues[name] = value;
			
			this.target[name] = value;
			
			this.target.onChange(delta);
		},
		"delete": function(name) {
			...
			this.target.onBeforeDelete(delta);
			
			delete this.target[name];
			
			this.target.onDelete(delta);
...
var Class = Base.extend({
	...
	constructor: function(config){
		...
		Object.defineProperties(this,{
            ...
            /**
             * @cfg {Boolean} [strictData=false]
             * When set to `true`, the #getData method will only return values for the attributes defined in #enumerableProperties.
             */
            explicitData: {
			...
			/**
			 * @property
			 * Tracks the original config values. This is only used for non-create CRUD functionality. 
			 * @private
			 */
			originalConfig: {
			...
			/**
			 * @property {Object}
			 * Automatically generates a model with a harmony proxy wrapper.
			 */
			EXTENDEDMODEL: {
				enumerable:	false,
				get:		function(){
		...
		this.on('readyForProxy',function(){
			if (me.id) {
				me.load();
			} else {
				me.monitor();
	...
	/**
	 * @method
	 * Get a model from the cross references. Cross references are stored when the #serialize method
	 * encounters a nested __NGN.model. #save and #load use #serialize.
	 * @param {Number/String}
	 * The reference number or full xref string (i.e. `xref::1`)
	 * @returns {__NGN.model.Model}
	 */
	getCrossReference: function(ref) {
		if (typeof ref == 'string')
			ref = parseInt(ref.split('::')[1]);
		if (ref-1 <= 0 || ref-1 > this._ref.length){
			this.onNoReference(ref);
			return null;
		}
		return this._ref[ref-1];
	},
	...
	extension: function() {
		...
		if (!__NGN.OBJECT_PROXY_SUPPORT)
			this.fireWarning('Model '+this.type+' does not have proxy support. To enable, launch the app as:\n'+('node '+'--harmony_proxies '.bold+process.mainModule.filename).yellow+'\n');
		
		if (this._xtn || !__NGN.OBJECT_PROXY_SUPPORT)
			return this;
		...
		wrap.prototype = proxyHandler;
		
		Proxy = Proxy || {create: function(){ return me; }};
		
		return Proxy.create(new wrap(this),me);
