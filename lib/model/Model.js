...
// The proxy handler prototype responsible for managing data events.
var arrayHandler = {
    get: function(obj, prop, index){
    	console.log('GET '.green, prop, index);
    	/*
		if (!isNaN(name))
					return stack[name];
				if(typeof stack[name] == "function"){
					...
					//if (name == 'inspect')
					//	return stack.prototype;
					console.log('reached undefined for '+name)
										 return stack[name];
				}*/
		return [];
    },
    set: function(receiver, name, value){
    	console.log('SET'.green,name,value);
    	stack[name] = value;
    }
...
var simpleHandler = {
	...
	get: function(obj,prop){
		...
		if (obj[prop] == undefined){
			// Determine whether the request is a getter
			prop = (	obj[prop.replace(/get/i,'')] !== undefined 
						? prop.replace(/get/i,'') 
						: (
							obj[__$.uncapitalize(prop.replace(/get/i,''))] !== undefined 
							? __$.uncapitalize(prop.replace(/get/i,'')) 
							: prop
						)
					);
			if (obj[prop] !== undefined)
		...
		return obj[prop] || this[prop] || undefined;
	},
	
	// SET trap
	set: function(obj,name,value){
		console.log(('Set '+name+' to ').magenta,value);
		
		// Create an array proxy for any array values
		/*if (Array.isArray(value)) {
			value = Proxy(value,arrayHandler);
		}*/
...
var Class = Base.extend({
	...
	getChangeLog: function(){
		if (!__NGN.OBJECT_PROXY_SUPPORT)
			this.fireWarning('Model '+this.type+'#getChangeLog cannot run because proxies are not supported. To enable, launch the app as:\n'+('node '+'--harmony_proxies '.bold+process.mainModule.filename).yellow+'\n');
	...
	rollback: function(changes){
		if (!__NGN.OBJECT_PROXY_SUPPORT){
			this.fireWarning('Model '+this.type+'#rollback cannot run because proxies are not supported. To enable, launch the app as:\n'+('node '+'--harmony_proxies '.bold+process.mainModule.filename).yellow+'\n');
			return;
		}
		changes = changes || 1;
		...
		for (var i=0; i<changes; i++){
			...
			switch(delta.type.trim().toLowerCase()){
				case 'create':
					delete this['___rollback::'+delta.property];
					this.changelog.pop();
					this.onRollback(delta);
					break;
				case 'update':
					this['___rollback::'+delta.property] = delta.oldValue;
					this.changelog.pop();
					this.onRollback(delta);
					break;
				case 'delete':
	...
	extension: function() {
		...
		var rtn = Proxy(this,simpleHandler);
