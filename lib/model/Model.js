...
var Class = Base.extend({
		...
		for (var field in this.fields){
			if (['id'].indexOf(field) < 0) {
				...
				if (this.fields[field] instanceof __NGN.model.data.Association) {
					...
					this._xref[field] = this.fields[field] || null;
	...
	validate: function(attribute){
		...
		var assc = Object.keys(this._xref);
					...
					if (dsc.get && !this.fields[key] instanceof __NGN.model.data.Association){
						...
						switch (typeof dsc.value) {
							...
							case 'object':
								...
								var isXRef = this._xref.hasOwnProperty(key);
	...
	getChanges: function(serializeAssociations){
		...
		//if (this.isNew){
			...
			if (Object.keys(this._xref).length > 0) {
				for (var field in this.fields){
					if (this._xref.hasOwnProperty(field) && this[field] !== null && this[field] !== undefined){
