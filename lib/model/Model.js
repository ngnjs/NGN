...
var Class = Base.extend({
	...
	constructor: function(config){
		...
		Object.defineProperties(this,{
			...
			/**
			 * @cfg {Boolean} [onDelete=true]
			 * Delete this object using #CRUD when this model is a nested child
			 * of a parent model.
			 */
			onParentDelete: {
				value:		__NGN.coalesce(config.onParentDelete,true),
				enumerable:	false,
				writable:	true
			},
			...
			/**
			 * @property
			 * Tracks the original data values. This is only used for non-create CRUD functionality. 
			 * @private
			 */
			originalConfig: {
				value:		{},
	...
	/**
	 * @method
	 * Get a model from the cross references. Cross references are stored when the #toData method
	 * encounters a nested model. #save and #load use #toData.
	 * @param {Number/String}
	 ...
	 * 
	 * If a value is an instance of MODEL.Model (i.e. a nested model or array of models), reference string is returned in the data.
	 * The model itself can be returned using #getXRef.
	 * @param {Object} [obj]
	 * Defaults to this object.
	 * @param {Boolean} [serializeXRefData=false]
	 * When set to `true`, any cross referenced data in nested models will be retrieved
	 * and the cross reference will be ignored. Use this when a complete JSON dump is
	 * required.
	 * @returns {Object}
	 * @protected
	 */
	toData: function(obj,serializeXRefData){
	
		if (obj instanceof Boolean){
			serializeXRefData = obj;
			obj = this;
		}
		
		serializeXRefData = __NGN.coalesce(serializeXRefData,false);
			...
			if ((_obj.hasOwnProperty(key) && (_obj.nonDataProperties.indexOf(key) < 0 && /^[a-z0-9 ]$/.test(key.substr(0,1)))) || (_obj[key] !== undefined && _obj.dataProperties.indexOf(key) >= 0)) {
				if ((this.explicitData && this.dataProperties.indexOf(key) >= 0) || !this.explicitData) {					
					if ((this.strictData && this._coreAttributes.indexOf(key) >= 0) || !this.strictData){
						...
						if (dsc.get){
							try {
								rtn[key] = typeof _obj[key] == 'object' ? _obj[key].toData(serializeXRefData) : _obj[key];
							} catch(e) {}
						} else if (!dsc.set) {
							switch (typeof dsc.value) {
								...
								case 'object':
									...
									if (Array.isArray(_obj[key])){				
										for (var i=0;i<_obj[key].length;i++){
											if (_obj[key][i] instanceof MODEL.Model){
												if (!serializeXRefData){
													me._ref[Object.keys(me._ref).length+1] = _obj[key][i];
													_obj[key][i] = 'xref::'+Object.keys(me._ref).length;
												} else
													_obj[key][i] = _obj[key][i].toData(serializeXRefData);
											}
										}
									}
									...
									if (isModel){
										if (!serializeXRefData){
											me._ref[Object.keys(me._ref).length+1] = _obj[key];
											rtn[key] = 'xref::'+Object.keys(me._ref).length;
										} else
											rtn[key] = _obj[key].toData(serializeXRefData);
									} else if (_obj[key]){
										struct = _obj[key].hasOwnProperty('toData') == true ? _obj[key].toData(serializeXRefData) : _obj[key];	
