...
var Class = Base.extend({
	...
	create: function(model,callback){
		...
		// Add the id -> _id mapping for MongoDB
		if (!this.map[model.type].hasOwnProperty('id'))
			this.addDataMapping(model.type,'id','_id');
		
		this.client.collection(model.type,{safe:true},function(err,collection){
			collection.insert(model.getData(),function(){
				callback();
				me.onCreate();
			});
		});
