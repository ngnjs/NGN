...
/**
 * @class NGN.system.Process
 * @singleton
 * Creating an process enables the globally accessible `process.ngn` variable,
 * which is designed to store custom logic. This class registers the
 * process, loads defaults, and provides several convenience methods
 * The `process.ngn` scope is a globally accessible variable designed to store 
 * custom application/process logic.
 */
...
var Class = Base.extend({
    ...
    /**
     ...
     /**
	 * @cfg {Object}
	 * Specify the default NGN.CRUD object(s) used by any {@link MODEL Model} objects.
	 * CRUD operations can be specified for a specific model(s) or all models.
	 * 
	 * **Example**
	 * 		NGNA.CRUD(new NGNX.datasource.CRUD.MongoDB('myAppDatasource'));
	 * or
	 * 		NGNA.CRUD({
	 ...
	 *		});
	 */
    setCRUD: function(map){
    	if (map.hasOwnProperty('default'))
    		this.CRUD['default'] = map['default'];
    	else {
    		if (map.hasOwnProperty('_isCRUD'))
    			this.CRUD['default'] = map;
    		else {
    			for (var crud in map){
    				if (map.hasOwnProperty(crud))
	    				this.CRUD[crud] = map[crud];
    			}
    		}
