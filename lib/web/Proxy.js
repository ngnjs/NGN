...
var Class = Base.extend({
	 ...
	 * @param {Object} config
	 * The configuration of the NGN.web.proxy.VirtualHost object.
	 */
	createVirtualHost: function(obj) {
		...
		this.vhost[obj.port][obj.hostname.toLowerCase()] = new __NGN.web.proxy.VirtualHost(obj);
	...
	getConfiguration: function(){
		...
		for (var i=0;i<vhosts.length;i++){
			for (var host in vhosts[i]) {
				...
				// Get the rules
				var r = {};
				for (var method in vh.rules){
					if (vh.rules.hasOwnProperty(method)){
					
						r[method] = r[method] || [];

						for (var n=0;n<vh.rules[method].length;n++) {
							var rl = {};
							
							// Get the pattern & strip the RegExp forward slashes
							var ptrn = vh.rules[method][n].pattern.toString().replace(/\/{1}/,'');
							ptrn = ptrn.substr(0,ptrn.length-1);
							
							rl[ptrn] = 	vh.rules[method][n].substitute
										+ (vh.rules[method][n].last == true ? '-last': '');
