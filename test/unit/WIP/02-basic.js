...
test('NGN.mixin', function (t) {
  ...
  t.ok(
    ...
    'NGN.mixin applies source descriptors to a target object.'
  )
...
test('NGN.forceNumber', function (t) {
  const dt = new Date(2000, 1, 1, 0, 0, 0, 0).getTime()
...
test('NGN.wrap', function (t) {
  ...
  const wrappedFn = NGN.wrap(() => { pre = true }, fn)
...
test('NGN.objectMissingProperties', function (t) {
  const obj = {
    a: 1,
    b: 2,
    c: 3,
  ...
  const missing = NGN.objectMissingProperties(obj, 'a', 'b', 'c', 'd', 'e', 'f')
  t.ok(missing[0] === 'd' && missing[1] === 'f', 'Returns list of missing attributes.')

  t.end()
})

test('NGN.objectExtraneousProperties', function (t) {
  const obj = {
    a: 1,
    b: 2,
    c: 3,
  ...
  const extra = NGN.objectExtraneousProperties(obj, 'a', 'b', 'c', 'e')
...
test('NGN.objectHasAll', function (t) {
  ...
  const check = NGN.objectHasAll(obj, 'a', 'b', 'c')
  const badCheck = NGN.objectHasAll(obj, 'a', 'b', 'c', 'd')
...
test('NGN.objectHasAny', function (t) {
  ...
  const check = NGN.objectHasAny(obj, 'a', 'b', 'c', 'd')
  const badCheck = NGN.objectHasAny(obj, 'x', 'y', 'z')
...
test('NGN.objectHasExactly', function (t) {
  ...
  const justRight = NGN.objectHasExactly(obj, 'a', 'b', 'c')
  const tooMany = NGN.objectHasExactly(obj, 'a', 'b', 'c', 'd')
  const tooFew = NGN.objectHasExactly(obj, 'a', 'b')
...
test('NGN.deprecate (method)', function (t) {
  const fn = function () {
    t.pass('Deprecated method executed.')
    t.end()
  }

  const tmpFn = NGN.deprecate(fn)

  tmpFn()
})

test('NGN.deprecate (class)', function (t) {
  class tmpClass {
    constructor () {
      t.pass('Deprecated method executed.')
      t.end()
  ...
  const tempClass = NGN.deprecateClass(tmpClass)

  let x = new tempClass() // eslint-disable-line
})

test('NGN.getPrimitive', function (t) {
  t.ok(NGN.getPrimitive('number') === Number, 'NGN.getPrimitive identifies number.')
  t.ok(NGN.getPrimitive('regexp') === RegExp, 'NGN.getPrimitive identifies regexp.')
  t.ok(NGN.getPrimitive('regex') === RegExp, 'NGN.getPrimitive identifies regex w/ warning.')
  t.ok(NGN.getPrimitive('boolean') === Boolean, 'NGN.getPrimitive identifies boolean.')
  t.ok(NGN.getPrimitive('symbol') === Symbol, 'NGN.getPrimitive identifies symbol.')
  t.ok(NGN.getPrimitive('date') === Date, 'NGN.getPrimitive identifies date.')
  t.ok(NGN.getPrimitive('array') === Array, 'NGN.getPrimitive identifies array.')
  t.ok(NGN.getPrimitive('object') === Object, 'NGN.getPrimitive identifies object.')
  t.ok(NGN.getPrimitive('function') === Function, 'NGN.getPrimitive identifies function.')
  t.ok(NGN.getPrimitive('string') === String, 'NGN.getPrimitive identifies string.')
  t.ok(NGN.getPrimitive('nada', String) === String, 'NGN.getPrimitive identifies default type.')
  t.ok(NGN.getPrimitive('nada') === undefined, 'NGN.getPrimitive defaults to undefined when no type is recognized.')
